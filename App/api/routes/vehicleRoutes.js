...
router.post('/vehicles', async (req, res) => {
  const { id, make, category, model, price, availability, year, plateNumber, color, damages } = req.body;

  try {
    const result = await pool.query`INSERT INTO Vehicle (ID, Make, Category, Model, Price, Availability, Year, Plate_Number, Color, Damages) VALUES (${id}, ${make}, ${category}, ${model}, ${price}, ${availability}, ${year}, ${plateNumber}, ${color}, ${damages})`;
...
router.put('/vehicles/:id', async (req, res) => {
  const vehicleId = req.params.id;
  const { make, category, model, price, availability, year, plateNumber, color, damages } = req.body;

  try {
    const result = await pool.query`UPDATE Vehicle SET Make = ${make}, Category = ${category}, Model = ${model}, Price = ${price}, Availability = ${availability}, Year = ${year}, Plate_Number = ${plateNumber}, Color = ${color}, Damages = ${damages} WHERE ID = ${vehicleId}`;
